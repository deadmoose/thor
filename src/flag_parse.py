...
#And it will produce:
#A timestamp of when the fuction was executed in Unitvesal Time in a form of struct_time
...
def flag_parse(page):
	...
	for x in curr_t[:-1]:
		...
		n = x[0][x[0].index("false\">")+7:x[0].index('</a>')]
		p = [0, 0, 0, 0, 0, 0]
		x = x[2:-1]
	...
	final = (timest, page_type, flag_name, founded, fame, pub_stm, rep, relay, isld_ctrl, royal, flg_crw, ext_stm)
	return final
